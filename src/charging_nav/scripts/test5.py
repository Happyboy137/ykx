...
import threading
import math
from unitree_sdk2py.go2.sport.sport_client import SportClient  # 导入 SportClient
...
def squat_motion(self):
    rospy.loginfo("Attempting to squat.")
    
    # 进入阻尼模式
    self.sport_client.Damp()  # 切换到阻尼模式
    rospy.loginfo("Entered Damp mode.")
    ...
    rospy.loginfo("Squat motion initiated.")
    time.sleep(SQUAT_DURATION)  # 持续下蹲2秒
    
    # 在这里你可以检查是否执行成功，或者进一步的状态更新
    # 例如，回到正常的运动模式
    self.sport_client.RiseSit()  # 恢复站立状态
    rospy.loginfo("Squat motion finished, returning to normal mode.")
    self.sport_client.Move(0, 0, 0)  # 停止运动


# --------------------------
# 增加精细的误差阈值
# --------------------------
def check_target_accuracy(self, target_pose, current_pose, tolerance=0.05):
    """
    比较当前位置和目标位置的误差，如果误差小于容忍度，则认为到达目标
    """
    ...
    if dist < tolerance:
        rospy.loginfo(f"Reached target within tolerance: {tolerance} meters")
        return True
    return False
...
class ChargingNavigator:
    def __init__(self):
        ...
        self.sport_client.SetTimeout(10.0)  # 设置超时时间
        self.sport_client.Init()  # 初始化 SportClient
